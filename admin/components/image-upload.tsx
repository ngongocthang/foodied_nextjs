...
const ImageUpload = ({disabled, onChange, onRemove, value}: ImageUploadProps) => {
    ...
    const onUpload = async (e: any) => {
        ...
        const uploadTask = uploadBytesResumable(
            ref(storage, `Images/${Date.now()}-${file.name}`), file, {contentType: file.type}
        )
        ...
        <div className="mb-4 flex items-center gap-4">
            {value.map(url => (
                <div className=" relative w-52 h-52 rounded-md overflow-hidden" key={url}>
                    <Image 
                    fill 
                    className="object-cover" alt="Billboard Image" 
                    src={url}
                    />
            ...
            <div className="w-52 h-52 rounded-md overflow-hidden border-dashed border border-gray-200 flex items-center justify-center flex-col gap-3">
                {isLoading ? (
                    <>
