...
export const PopularContent = ({data}: PopularContentProps) => {
    ...
    <Card className="w-full max-h-[340px] rounded-md bg-white shadow-lg border-none flex flex-col items-center justify-center relative py-6 pt-24 md:pt-28">
        ...
        <div className=" w-full flex items-center justify-center gap-2 flex-wrap px-2 mt-4">
            ...
            {data.cuisine && (
                <div className="rounded-md bg-emerald-500/10 px-2 py-[2px] text-[10px] capitalize ">
                    {data.cuisine }
                </div>
            )}
            {data.category && (
                <div className="rounded-md bg-blue-500/10 px-2 py-[2px] text-[10px] capitalize ">
                    {data.category }
                </div>
            )}
            {data.kitchen && (
                <div className="rounded-md bg-red-500/10 px-2 py-[2px] text-[10px] capitalize ">
                    {data.kitchen }
                </div>
            )}
            {data.size && (
                <div className="rounded-md bg-yellow-500/10 px-2 py-[2px] text-[10px] capitalize ">
                    {data.size }
                </div>
            )}
            </div>
            ...
            Food product quality: safe, tasty, nutritious, fresh, attractive packaging.
            </CardDescription>
            ...
            {/* add to wishlist  */}
            <Button
            className="absolute left-0 top-0 hover:bg-transparent" variant={"ghost"}
            >
                <IsLikedIcon
